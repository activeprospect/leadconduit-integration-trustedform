...
const handle = (vars, callback) => {
  ...
  if(!_.isArray(vars.trustedform.required_text)) vars.trustedform.required_text = [vars.trustedform.required_text];

  let options = {
    cert_url: certUrl,
    vendor: _.get(vars, 'trustedform.vendor') || vars.source.name,
    required_text: vars.trustedform.required_text
  };

  client.claim(options, (err, res, body) => {
    if(err) callback(undefined, { outcome: 'error', reason: err });

    callback(undefined, parseResponse(res, body));
  });
