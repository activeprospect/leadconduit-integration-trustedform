...
const handle = (vars, callback) => {
  ...
  if(trustedform.required_text && !_.isArray(trustedform.required_text)) trustedform.required_text = [trustedform.required_text];
  if(trustedform.forbidden_text && !_.isArray(trustedform.forbidden_text)) trustedform.forbidden_text = [trustedform.forbidden_text];

  let options = {
    cert_url: certUrl,
    vendor: _.get(vars, 'trustedform.vendor') || vars.source.name,
    required_text: trustedform.required_text,
    forbidden_text: trustedform.forbidden_text,
    email: lead.email,
  ...
  client.claim(options, (err, res, body) => {
    if(err) callback(null, { outcome: 'error', reason: err });
...
const parseResponse  = (res, body, vars) => {
  ...
  if (res.statusCode === 201 && event.cert) {
    ...
    appended = {
      outcome: 'error',
      reason:  `TrustedForm error - ${errorMessage} (${res.statusCode})`
    };
  }
