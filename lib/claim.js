...
const handle = (vars, callback) => {
  ...
  const options = {
    cert_url: certUrl,
    vendor: get(vars, 'trustedform.vendor') || vars.source.name,
  ...
  if (get(vars, 'trustedform.use_v3_api')) {
...
const requestVariables = () => [
  ...
  { name: 'trustedform.use_v3_api', type: 'boolean', required: false, description: 'Set to true to use version 3.0 of the TrustedForm API' },
...
const parseResponse = (res, body, vars) => {
  ...
  if (statusCode === 201) {
    const found = (event.scans && event.scans.found) ? uniq(event.scans.found) : [];
    const notFound = (event.scans && event.scans.not_found) ? uniq(event.scans.not_found) : [];
    ...
    // parse full data from pre-v3 API response
    if (event.cert) {
      const hostedUrl = get(event, 'cert.parent_location') || get(event, 'cert.location');
      const oldClaimData = {
        is_masked: event.masked,
        expires_at: event.expires_at,
        share_url: event.share_url,
        user_agent: get(event, 'cert.user_agent'),
        browser: get(event, 'cert.browser'),
        os: get(event, 'cert.operating_system'),
        ip: get(event, 'cert.ip'),
        token: get(event, 'cert.token') || get(event, 'cert.cert_id'),
        location: {
          city: get(event, 'cert.geo.city', undefined),
          country_code: get(event, 'cert.geo.country_code', undefined),
          latitude: get(event, 'cert.geo.lat', undefined),
          longitude: get(event, 'cert.geo.lon', undefined),
          postal_code: get(event, 'cert.geo.postal_code', undefined),
          state: get(event, 'cert.geo.state', undefined),
          time_zone: get(event, 'cert.geo.time_zone', undefined)
        },
        snapshot_url: get(event, 'cert.snapshot_url'),
        url: hostedUrl,
        domain: (hostedUrl) ? new URL(hostedUrl).hostname : null,
        website: {
          location: get(event, 'cert.location'),
          parent_location: get(event, 'cert.parent_location')
        },
        age_in_seconds: ageInSeconds(event),
        time_on_page_in_seconds: timeOnPageInSeconds(get(event, 'cert.event_duration')),
        created_at: get(event, 'cert.created_at'),
        warnings: event.warnings
      };

      const requiredStrings = get(vars, 'trustedform.scan_required_text', null);
      ...
      // 'warnings' is also only an attribute in pre-v3 responses
      if (event.warnings) {
      ...
      Object.assign(appended, oldClaimData);
    }
