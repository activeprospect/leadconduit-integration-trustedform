...
const content = (vars) => {
  ...
  params.email   = (vars.lead.email) ? vars.lead.email.toString() : null;
  params.phone_1 = (vars.lead.phone_1) ? vars.lead.phone_1.toString() : null;
  params.phone_2 = (vars.lead.phone_2) ? vars.lead.phone_2.toString() : null;
  params.phone_3 = (vars.lead.phone_3) ? vars.lead.phone_3.toString() : null;
...
const request = (vars) => {
  ...
  const certUrl = (vars.lead.trustedform_cert_url && vars.lead.trustedform_cert_url.includes(',')) ?
...
const response  = (vars, req, res) => {
  ...
  if (res.status === 201 && event.cert) {
    ...
    let duration = res.headers['X-Runtime'];
