...
const handle = (vars, callback) => {
  ...
  let host = (process.env.NODE_ENV === 'production') ? 'https://next.leadconduit.com' : 'https://next.leadconduit-staging.com';
  let options = {
    cert_url: certUrl,
...
const validate = (vars) => {
  const isNotProduction = (process.env.NODE_ENV === 'staging' || process.env.NODE_ENV === 'development');
  ...
  const stagingValid = (isNotProduction && (certUrl.match(tfStagingRegex) || certUrl.match(tfStagingFacebook)));
