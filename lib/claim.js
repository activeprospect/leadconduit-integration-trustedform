...
const { ageInSeconds, timeOnPageInSeconds, formatScanReason, countRequiredMatched, evalFingerprint } = require('./helpers.js');
const Client = require('trustedform-api-client');

const handle = (vars, callback) => {

  const apiKey = (_.get(vars, 'trustedform.api_key'))
  ...
  let host = (process.env.NODE_ENV === 'production') ? 'https://next.trustedform.com' : 'https://next.staging.trustedform.com';
  let options = {
    cert_url: certUrl,
...
const validate = (vars) => {
  ...
  // example: same as above but with staging
  ...
  const isProductionValid = (isProduction && !certUrl.match(tfRegex) && !certUrl.match(tfFacebookRegex));
  const isStagingValid = (!certUrl.match(tfStagingRegex) && !certUrl.match(tfStagingFacebook)) || isProductionValid;

  if (!isProductionValid && !isStagingValid) return 'TrustedForm cert URL must be valid';
};
...
const parseResponse  = (res, body, vars) => {
  ...
  if (statusCode === 201 && event.cert) {
    ...
    appended = {
      outcome: 'error',
      reason:  `TrustedForm error - ${errorMessage} (${statusCode})`
    };
  }
