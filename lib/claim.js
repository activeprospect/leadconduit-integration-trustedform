...
const { validate, ageInSeconds, timeOnPageInSeconds, formatScanReason, countRequiredMatched, evalFingerprint, configureClaimOptions } = require('./helpers.js');
...
const parseResponse = (res, body, vars) => {
  ...
  if (statusCode === 201 && event.cert) {
    ...
    if (statusCode === 410) {
      // 410 errors do not return a message, so add one
      ...
      // if .message is present, use that instead
