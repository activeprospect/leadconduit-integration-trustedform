...
const countRequiredMatched = (requiredStrings, scans) => {
  if (!requiredStrings || !scans) return undefined;
  // normalize data
...
const validate = (vars) => {
  ...
  const stagingValid = (!['production', 'development'].includes(process.env.NODE_ENV) && (tfStagingRegex.test(getUrlString(certUrl)) || tfStagingFacebook.test(getUrlString(certUrl))));
...
const getLeadConduitUrl = (env = process.env.NODE_ENV) => {
  switch (env) {
    ...
    case 'test': return 'https://app.leadconduit-staging.com';
