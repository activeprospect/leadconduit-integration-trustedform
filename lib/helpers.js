...
const ageInSeconds = (event) => {
  const timeOnPage = timeOnPageInSeconds(_.get(event, 'cert.event_duration')) || 0;
  const certAge = (new Date(event.created_at) - new Date(_.get(event, 'cert.created_at'))) / 1000;
