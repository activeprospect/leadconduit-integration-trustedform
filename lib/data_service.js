...
const request = (vars) => {
  ...
  let body = {
    'scan[]': trustedform.scan_required_text,
    'scan[]!': trustedform.scan_forbidden_text,
  ...
  if (res.status === 201) {
    ...
    return {
      data_service: {
        ...
        matching: get(event, 'fingerprints.matching'),
        non_matching: get(event, 'fingerprints.non_matching'),
        masked: event.masked,
        found: found,
        not_found: notFound,
...
response.variables = () => [
  ...
  { name: 'data_service.matching', type: 'array', description: 'Matching fingerprints that were found on the TrustedForm certificate' },
  { name: 'data_service.non_matching', type: 'array', description: 'Non-matching fingerprints that were found on the TrustedForm certificate' },
  { name: 'data_service.masked', type: 'boolean', description: 'Whether or not the TrustedForm certificate is masked' },
  { name: 'data_service.found', type: 'array', description: 'Language that was found through the use of page scanning' },
  { name: 'data_service.not_found', type: 'array', description: 'Language that was not found through the use of page scanning' },
