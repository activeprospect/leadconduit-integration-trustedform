...
const response = (vars, req, res) => {
  ...
  if (res.status === 201) {
    ...
    if (cert.consented_at) {
      response.data_service.consented_at = cert.consented_at;
      response.consented_at = cert.consented_at;
    }
    return response;
...
response.variables = () => [
  ...
  { name: 'data_service.billable', type: 'number', description: 'If the event is billable, the billable count for the event, else 0' },
  ...
  { name: 'data_service.consented_at', type: 'time', description: 'Time the user checked the consent language checkbox, in UTC ISO8601 format' },
  ...
  { name: 'data_service.expires_at', type: 'time', description: 'Timestamp indicating when the claim period for the TrustedForm certificate expires' },
  { name: 'data_service.form_input_method', type: 'array', description: 'The input method or methods the consumer used to fill out the form: autofill, paste, typing' },
  { name: 'data_service.framed', type: 'boolean', description: 'Whether or not the page_url was in an iframe' },
  { name: 'data_service.has_consented', type: 'boolean', description: 'Whether or not the user checked the consent language checkbox' },
