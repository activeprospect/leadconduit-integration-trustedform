...
const parseResponse = (statusCode, body, vars) => {
  ...
  if (statusCode === 201) {
    ...
    if (_.get(event, 'scans.forbidden_found')) appended.forbidden_scans_found = _.get(event, 'scans.forbidden_found');
    if (_.get(event, 'scans.forbidden_not_found')) appended.forbidden_scans_not_found = _.get(event, 'scans.forbidden_not_found');
    if (_.get(event, 'scans.required_found')) appended.required_scans_found = _.get(event, 'scans.required_found');
    if (_.get(event, 'scans.required_not_found')) appended.required_scans_not_found = _.get(event, 'scans.required_not_found');

    if (_.get(vars, 'trustedform.scan_required_text')) {
      // 'all', 'some', or 'none'
      appended.num_required_matched = countRequiredMatched(_.get(vars, 'trustedform.scan_required_text'), appended.required_scans_found);
    }
  } else {
    // handle all non-201 statuses
    let errorMessage = _.get(event, 'errors.detail');
