    ...
    try {
      ...
      if (res.status === 200) {
        return pickBy({
          ...
          expires_at: roundTimestamp(parsed.retain?.results?.expires_at || parsed.insights?.properties?.expires_at),
          ...
          created_at: roundTimestamp(parsed.insights?.properties?.created_at),
...
const roundTimestamp = (time) => {
  if (!time) return undefined;
  // check if there is a decimal value at the end of the timestamp
  const parsed = time.match(/(.*)\.([0-9]+)Z$/);
  if (parsed) {
    // round the decimal value to 4 places
    return `${parsed[1]}.${parsed[2].slice(0,4)}Z`
  } else {
    return time;
  }
}
