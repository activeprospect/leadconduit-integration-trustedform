...
const validate = (vars) => {
  ...
  if (!vars.trustedform?.retain?.valueOf() && (formatProperties(vars.trustedform).length === 0 || vars.trustedform.page_scan?.valueOf())) return 'no properties selected for TrustedForm Insights';
}
    ...
    try {
      ...
      if (res.status === 200) {
        return pickBy({
          ...
          reference: parsed.retain?.reference,
...
response.variables = () => [
  ...
  { name: 'reference', type: 'string', description: 'The parameter provided in the request, intended to be a reference to help you identify the lead associated with the certificate.' },
