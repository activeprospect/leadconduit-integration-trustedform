...
const request = (vars) => {
  ...
  if (trustedform.insights.valueOf()) {
    body.insights = {
      properties: formatProperties(trustedform)
    }
    if (trustedform.page_scan?.valueOf()) {
      body.insights.required = trustedform.scan_required_text;
      body.insights.forbidden = trustedform.scan_forbidden_text;
      body.insights.delimiter = trustedform.scan_delimiter;
    }
  }
...
const formatProperties = (trustedform) => {
  const insightsProps = [
  ...
  let selectedProps = insightsProps.map((field) => {
    if (trustedform[field.mapping]?.valueOf()) return field.value;
  });
...
request.variables = () => [
  ...
  { name: 'trustedform.retain', type: 'boolean', required: true, description: 'Check TrustedForm Retain?' },
  ...
  { name: 'trustedform.insights', type: 'boolean', required: true, description: 'Check TrustedForm Insights?' },
  { name: 'trustedform.age', type: 'boolean', required: false, description: 'Request TrustedForm Insights age data?' },
  { name: 'trustedform.browser', type: 'boolean', required: false, description: 'Request TrustedForm Insights browser data?' },
  { name: 'trustedform.created_timestamp', type: 'boolean', required: false, description: 'Request TrustedForm Insights created timestamp data?' },
  { name: 'trustedform.domain', type: 'boolean', required: false, description: 'Request TrustedForm Insights domain data?' },
  { name: 'trustedform.expiration_timestamp', type: 'boolean', required: false, description: 'Request TrustedForm Insights expiration timestamp data?' },
  { name: 'trustedform.framed', type: 'boolean', required: false, description: 'Request TrustedForm Insights framed data?' },
  { name: 'trustedform.form_input_method', type: 'boolean', required: false, description: 'Request TrustedForm Insights form input method data?' },
  { name: 'trustedform.form_input_kpm', type: 'boolean', required: false, description: 'Request TrustedForm Insights form input KPM data?' },
  { name: 'trustedform.form_input_wpm', type: 'boolean', required: false, description: 'Request TrustedForm Insights form input WPM data?' },
  { name: 'trustedform.ip_address', type: 'boolean', required: false, description: 'Request TrustedForm Insights IP address data?' },
  { name: 'trustedform.location', type: 'boolean', required: false, description: 'Request TrustedForm Insights location data?' },
  { name: 'trustedform.masked', type: 'boolean', required: false, description: 'Request TrustedForm Insights masked data?' },
  { name: 'trustedform.sensitive_content', type: 'boolean', required: false, description: 'Request TrustedForm Insights sensitive content data?' },
  { name: 'trustedform.sensitive_form_fields', type: 'boolean', required: false, description: 'Request TrustedForm Insights sensitive form fields data?' },
  { name: 'trustedform.operating_system', type: 'boolean', required: false, description: 'Request TrustedForm Insights operating system data?' },
  { name: 'trustedform.page_scan', type: 'boolean', required: false, description: 'Request TrustedForm Insights page scan data?' },
  { name: 'trustedform.page_url', type: 'boolean', required: false, description: 'Request TrustedForm Insights page URL data?' },
  { name: 'trustedform.parent_page_url', type: 'boolean', required: false, description: 'Request TrustedForm Insights parent page URL data?' },
  { name: 'trustedform.time_on_page', type: 'boolean', required: false, description: 'Request TrustedForm Insights time on page data?' },
...
const validate = (vars) => {
  ...
  if (!vars.trustedform?.retain?.valueOf() && formatProperties(vars.trustedform).length === 0) return 'no properties selected for TrustedForm Insights';
}
    ...
    try {
      ...
      if (res.status === 200) {
        return {
          ...
          previously_retained: parsed.retain?.result?.previously_retained,
          expires_at: parsed.retain?.result?.expires_at || parsed.insights?.properties?.expires_at,
